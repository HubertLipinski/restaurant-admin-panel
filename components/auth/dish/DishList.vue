...
const { data: response, pending, refresh } = await useLazyAsyncData<{ data: ApiResponse<Dish> }>('dishes', () => useApiFetch(store.apiPath, {
  query: {
    query: search.value,
    page: page.value,
...
watch(response, (response: ApiResponse<Dish>) => {
  if (response?.error) {
    errorResponse('dishes', response.error)
    return
  }
