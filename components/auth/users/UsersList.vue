...
const selectedRole = ref(roleFilter[0])
  ...
  [
    {
      ...
      click: async () => await navigateTo({ path: `/users/${row.id}/edit` }),
    },
  ],
  [
    {
...
const resetFilters = () => {
  search.value = ''
  selectedRole.value = roleFilter[0]
}
...
const { data: response, pending, refresh } = await useLazyAsyncData<{
...
}>('users', () => useApiFetch(store.apiPath, {
  query: {
  ...
  default: () => [],
  watch: [page, search, pageCount, selectedRole],
})
...
watch(response, (response: ApiResponse<User>) => {
  if (response?.error) {
    errorResponse('users', response.error)
    return
  }
...
<template>
  ...
  >
    ...
    <div class="flex items-center justify-between gap-3 px-4 py-3">
      ...
      <UFormGroup label="Rola" size="xs">
        <USelectMenu v-model="selectedRole"
                     :options="roleFilter"
    ...
    <div class="flex justify-between items-center w-full px-4 py-3">
      ...
      <div class="flex gap-1.5 items-center">
        ...
        <UButton
          ...
          color="gray"
          size="xs"
          :disabled="search === '' && selectedRole.value === null"
          @click="resetFilters"
        >
          Resetuj
        </UButton>
    ...
    <ConfirmModal
      ...
      :confirm="confirm"
      :cancel="cancel"
      prompt="Czy na pewno chcesz usunąć tego uzytkownika?"
    />
  </UCard>
</template>
